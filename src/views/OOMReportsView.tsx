...
import {
  useLocalDesktopHost,
  type DesktopHost,
  type DesktopOOMReport,
  type DesktopOOMReportFile,
...
import { MetadataCard, ReportShareDialog } from "./CrashReportsView";
...
function reportPath(name: string, recordedAt: number | null): string {
  const path = `tools/oom-reports/${encodeURIComponent(name)}`;
  return recordedAt !== null ? `${path}?at=${recordedAt}` : path;
}

function reportFilePath(name: string, file: string, recordedAt: number | null): string {
  const path = `tools/oom-reports/${encodeURIComponent(name)}/${encodeURIComponent(file)}`;
  return recordedAt !== null ? `${path}?at=${recordedAt}` : path;
}

export function oomReportTitle(name: string, recordedAt: number | null, language: string): string {
...
export function OOMReportListView() {
  ...
  return <OOMReportListContent host={host} />;
}

function OOMReportListContent({ host }: { host: DesktopHost }) {
  const api = useApi();
                ...
                {reports.length === 0 ? (
                  ...
                  reports.map((report) => (
                    <button
                      ...
                      onClick={() => navigate(reportPath(report.name, report.recordedAt))}
                    >
                      <span className={cx(crashStyles.reportDot, !report.isRead && crashStyles.unread)} />
                      <span className={crashStyles.reportText}>
                        <span className={cx(crashStyles.reportDate, !report.isRead && crashStyles.unread)}>
...
export function OOMReportDetailView(props: { name: string; recordedAt: number | null }) {
  ...
  return <OOMReportDetailContent host={host} name={props.name} recordedAt={props.recordedAt} />;
}

function OOMReportDetailContent({
  host,
  name,
  recordedAt,
}: {
              ...
              {files.map((file) =>
                file.isProfile ? (
                  ...
                  <button
                    key={file.name}
                    className="nav-row"
                    onClick={() => navigate(reportFilePath(name, file.name, recordedAt))}
                  >
    ...
    <div className="page">
      <PageHeader
        ...
        back={{
          ...
          onClick: () => navigate(reportPath(name, recordedAt)),
        }}
      />
      <div className="settings-stack">
        {content === null ? (
